...
/**
  ...
  * Дата: February 15, 2006
  *
  * Returns: да/false whether a path is normalized.
  *
  * Version:
  *
  * License: Public domain.
  ...
  *	 isNormPath(r"..\directory\file") => да
  * }
  * version(linux)
  * {
...
/**
  * Normalizes a path.
  *
  * This will normalize a path by collapsing redundant separators and родитель/current directory
...
body
{
	...
	if (path != sep && path != curdir && path != pardir)
	{
		...
		path = std.ткст.squeeze(path, sep);
		...
		pathComps = std.ткст.split(path, sep);
...
char[] normSep(char[] path)
{
	version(Windows)
	{
		...
		if (std.regexp.find(path, lSep) != -1)
		{
			path = std.ткст.replace(path, lSep, wSep);
