...
typedef цел EventType;

		//! \cond EXTERN
		static extern (C) EventType wxEvent_GetEventType(IntPtr сам);
		static extern (C) цел    wxEvent_GetId(IntPtr сам);
		static extern (C) бул   wxEvent_GetSkipped(IntPtr сам);
		static extern (C) цел    wxEvent_GetTimestamp(IntPtr сам);
		static extern (C) проц   wxEvent_Skip(IntPtr сам, бул skip);
		static extern (C) IntPtr wxEvent_GetEventObject(IntPtr сам);
		static extern (C) проц   wxEvent_SetEventObject(IntPtr сам, IntPtr object);
...
version(__WXMSW__){
		...
		alias static Событие function(IntPtr wxobj) newfunc;

		protected static newfunc[EventType] funcmap;
	
		//---------------------------------------------------------------------
		private static Событие Нов(IntPtr объ);
		public static проц ДобавьТипСоб(EventType evt, newfunc func);
		public this(IntPtr wxobj) ;
		public static Событие CreateFrom(IntPtr wxEvent);
		...
		public проц EventIntPtr(IntPtr ptr);
	}
