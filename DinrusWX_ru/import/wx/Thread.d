...
enum wxMutexType
{
	...
	//-----------------------------------------------------------------------------
		...
		//! \cond EXTERN
		static extern (C) IntPtr wxMutex_ctor(цел mutexType);
		static extern (C) проц wxMutex_dtor(IntPtr сам);
		static extern (C) бул wxMutex_IsOk(IntPtr сам);
		//! \endcond
...
//! a shared resource. Only one thread at a time can own a mutex object.
	...
	public class MutexLocker : wxObject
	{
		...
		//! \cond EXTERN
		static extern (C) IntPtr wxCriticalSection_ctor(цел mutexType);
		static extern (C) проц wxCriticalSection_dtor(IntPtr сам);
		static extern (C) проц wxCriticalSection_Enter(IntPtr сам);
		static extern (C) проц wxCriticalSection_Leave(IntPtr сам);
		//! \endcond
