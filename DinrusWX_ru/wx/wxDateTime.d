//-----------------------------------------------------------------------------
// wxD - ДатаВремя.d
// (C) 2005 bero <berobero@users.sourceforge.net>
// based on
// wx.NET - ДатаВремя.cs
//
/// The wxDateTime wrapper class.
//
// Written by Bryan Bulten (bryan@bulten.ca)
// (C) 2003 Bryan Bulten
// Licensed under the wxWidgets license, see LICENSE.txt for details.
//
// $Ид: wxDateTime.d,v 1.11 2007/04/19 19:44:05 afb Exp $
//-----------------------------------------------------------------------------

module wx.wxDateTime;
public import wx.common;
//private import std.дата;

alias wxDateTime ДатаВремя;
enum ДеньНед
{
    Sun, Mon, Tue, Wed, Thu, Fri, Sat, Inv_WeekDay
};

/* wxDateTime imprementation is class { longlong } */

//! \cond EXTERN
static extern (C) ЦелУкз wxDefaultDateTime_Get();
static extern (C) ЦелУкз wxDateTime_ctor();
static extern (C) ЦелУкз wxDateTime_Now();
static extern (C) проц   wxDateTime_dtor(ЦелУкз сам);
static extern (C) проц   wxDateTime_Set(ЦелУкз сам, ushort day, цел month, цел year, ushort hour, ushort minute, ushort second, ushort millisec);
static extern (C) ushort wxDateTime_GetYear(ЦелУкз сам);
static extern (C) цел    wxDateTime_GetMonth(ЦелУкз сам);
static extern (C) ushort wxDateTime_GetDay(ЦелУкз сам);
static extern (C) ushort wxDateTime_GetHour(ЦелУкз сам);
static extern (C) ushort wxDateTime_GetMinute(ЦелУкз сам);
static extern (C) ushort wxDateTime_GetSecond(ЦелУкз сам);
static extern (C) ushort wxDateTime_GetMillisecond(ЦелУкз сам);
//! \endcond

//-----------------------------------------------------------------------------

/// wxDateTime class represents an absolute moment in time.
public class wxDateTime : wxObject
{
    static wxDateTime wxDefaultDateTime;
    static this()
    {
        wxDefaultDateTime = new wxDateTime(wxDefaultDateTime_Get());
    }

    public this(ЦелУкз wxobj)
    {
        super(wxobj);
    }

    private this(ЦелУкз wxobj, бул memOwn)
    {
        super(wxobj);
        this.memOwn = memOwn;
    }

    public this()
    {
        this(wxDateTime_ctor(), да);
    }

    //---------------------------------------------------------------------

    override protected проц dtor()
    {
        wxDateTime_dtor(wxobj);
    }

    //-----------------------------------------------------------------------------

    public проц Уст(ushort day, цел month, цел year, ushort hour, ushort minute, ushort second, ushort millisec)
    {
        wxDateTime_Set(wxobj, day, month, year, hour, minute, second, millisec);
    }

    //-----------------------------------------------------------------------------

    public ushort Year()
    {
        return wxDateTime_GetYear(wxobj);
    }

    public цел Month()
    {
        return wxDateTime_GetMonth(wxobj);
    }

    public ushort Day()
    {
        return wxDateTime_GetDay(wxobj);
    }

    public ushort Hour()
    {
        return wxDateTime_GetHour(wxobj);
    }

    public ushort Minute()
    {
        return wxDateTime_GetMinute(wxobj);
    }

    public ushort Second()
    {
        return wxDateTime_GetSecond(wxobj);
    }

    public ushort Millisecond()
    {
        return wxDateTime_GetMillisecond(wxobj);
    }

    static wxDateTime Now()
    {
        return new wxDateTime(wxDateTime_Now());
    }
    //-----------------------------------------------------------------------------
    /+
    public static implicit operator ДатаВремя (wxDateTime wdt)
    {
        ДатаВремя dt = new ДатаВремя(wdt.Year, cast(цел)wdt.Month+1, cast(цел)wdt.Day,
                                   cast(цел)wdt.Hour, cast(цел)wdt.Minute,
                                   cast(цел)wdt.Second, cast(цел)wdt.Millisecond);
        return dt;
    }

    public static implicit operator wxDateTime (ДатаВремя dt)
    {
        wxDateTime wdt = new wxDateTime();
        wdt.Уст((ushort)dt.Day, dt.Month-1, dt.Year, (ushort)dt.Hour,
                (ushort)dt.Minute, (ushort)dt.Second,
                (ushort)dt.Millisecond);
        return wdt;
    }
    +/
    //-----------------------------------------------------------------------------
}
