...
public class StaticBitmap : Control
{
    ...
    public this(Window parent, int id, Bitmap label, Point pos = wxDefaultPosition, Size size = wxDefaultSize, int style = 0, string name = wxStaticBitmapNameStr)
    {
        ...
        {
            throw new InvalidOperationException("Не удалось создать StaticBitmap");
        }
    }

    public static wxObject New(IntPtr wxobj)
    {
        return new StaticBitmap(wxobj);
    }
    ...
    {
        wxStaticBitmap_SetBitmap(wxobj, wxObject.SafePtr(value));
    }
    public Bitmap bitmap()
    {
        return cast(Bitmap)FindObject(wxStaticBitmap_GetBitmap(wxobj), &Bitmap.New);
    }
}
