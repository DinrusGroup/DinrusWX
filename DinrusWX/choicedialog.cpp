...
public:
        ...
        const wxString *choices,
                ...
                long style,
                const wxPoint& pos)
        : wxSingleChoiceDialog(parent, message, caption, n, choices, (void**) clientData, style, pos)
    {}
                ...
                int width, int height)
{
	...
	if(width == NULL) width = wxCHOICE_WIDTH;
	if(height == NULL) height = wxCHOICE_HEIGHT;

    return wxGetSelectedChoices(*selections, wxstr(message),
                wxstr(caption),
                was, parent, x, y, centre,
                width, height);
